    ...
    <div class="relative w-full map-container shadow-sm overflow-hidden">
        ...
        <div id="canvassing-sidebar"
                ...
                <div class="p-5">
                    ...
                    <div id="main-controls" class="flex flex-col gap-3">
                        ...
                        <div class="relative group">
                            <input type="text" id="searchToko" oninput="renderList()"
            ...
            <div id="status-summary" class="mx-4 mb-2 grid grid-cols-2 gap-3 text-[11px] px-4 mt-2">
                <div
                    class="border border-green-100 dark:border-green-900/40 bg-green-50 dark:bg-green-900/20 rounded-none p-3 shadow-sm">
                    <div class="text-green-700 dark:text-green-300 font-bold uppercase tracking-wide">Sudah Cek</div>
                    <div id="checked-count" class="mt-1 text-2xl font-black text-green-800 dark:text-green-200">0</div>
                </div>
                <div
                    class="border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-slate-800 rounded-none p-3 shadow-sm">
                    <div class="text-gray-600 dark:text-gray-300 font-bold uppercase tracking-wide">Belum Cek</div>
                    <div id="unchecked-count" class="mt-1 text-2xl font-black text-gray-800 dark:text-white">0</div>
                </div>
            </div>
        ...
        <div class="absolute top-4 right-4 z-[1000] w-[calc(100%-2rem)] md:w-80 left-4 md:left-auto mobile-search-wrap">
            <div class="relative group">
                <input type="text" id="map_search_input"
                    ...
                    placeholder="Cari lokasi di peta..." oninput="handleMapSearch(this.value)">
...
@section('js')
    ...
    <script>
        ...
        function renderMarkers() {
            ...
            tokos.forEach(toko => {
                const status = toko.is_checked ?
                    { label: 'Sudah Cek', color: '#166534', bg: '#dcfce7' } :
                    { label: 'Belum Cek', color: '#374151', bg: '#f3f4f6' };

                const defaultFoto = window.location.origin + '/assets/1.png';
                const fotoUrl = toko.foto || defaultFoto;

                const markerColor = toko.is_checked ? '#22c55e' : '#ef4444';
                ...
                const contentString = `
                                                                                                <div class="p-2 font-sans max-w-[220px]">
                                                                                                    ...
                                                                                                    <div class="flex flex-col gap-2">
                                                                                                        <button onclick="window.toggleStatus(${toko.id})" class="block w-full text-center bg-red-600 text-white text-[11px] font-bold py-2 rounded-none transition-colors shadow-md shadow-blue-100">${btnLabel}</button>
        ...
        function renderList() {
            ...
            const searchInput = document.getElementById('searchToko').value.toLowerCase();
            container.innerHTML = '';

            let filteredTokos = tokos.filter(toko =>
                toko.nama.toLowerCase().includes(searchInput) ||
                toko.alamat.toLowerCase().includes(searchInput)
            );
            ...
            filteredTokos.forEach(toko => {
                ...
                const status = toko.is_checked ?
                    { label: 'Sudah Cek', bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400' } :
                    { label: 'Belum Cek', bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-700 dark:text-gray-300' };
                ...
                container.innerHTML += `
                                                                                                <div id="card-${toko.id}" class="border ${selectedClasses} dark:bg-black rounded-none p-4 hover:shadow-md bg-white dark:bg-slate-800 flex flex-col gap-1 cursor-pointer transition-all mb-3 relative" onclick="window.handleCardClick(${toko.latitude}, ${toko.longitude}, ${toko.id})">
                                                                                                    ...
                                                                                                    <div class="flex flex-col mt-3 gap-2">
                                                                                                        ...
                                                                                                        <button onclick="event.stopPropagation(); window.showProposedItems('${toko.kode_mitra}')" class="w-full btn  hover:bg-blue-700  py-2 rounded-none text-[10px] font-bold transition-colors uppercase tracking-widest shadow-sm">
        ...
        function updateStatusSummary(filteredTokos) {
            ...
            const checkedCount = filteredTokos.filter(toko => toko.is_checked).length;
            const uncheckedCount = filteredTokos.length - checkedCount;
        ...
        window.showProposedItems = async function (kodeMitra) {
            const toko = tokos.find(t => t.kode_mitra === kodeMitra);
            ...
            try {
                const res = await fetch(`/api/toko/${kodeMitra}/items`);
                ...
                if (json.status === 'success') {
                    ...
                    if (items.length > 0) {
                        items.forEach(item => {
                            ...
                            itemsHtml += `
                                                                                                            <div class="p-3 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-white/5 rounded-none mb-2 flex justify-between items-center transition-hover hover:bg-white dark:hover:bg-slate-700">
                                                                                                                <div class="flex flex-col">
                                                                                                                    <span class="text-[11px] font-extrabold text-gray-900 dark:text-white uppercase tracking-tight">${namaProduk}</span>
                                                                                                                    <span class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">Kode: ${item.kode_produk}</span>
                                                                                                                </div>
                                                                                                                <div class="text-right">
                                                                                                                    <span class="text-[11px] font-bold text-blue-600 dark:text-blue-400">Rp ${new Intl.NumberFormat('id-ID').format(item.harga)}</span>
                                                                                                                </div>
                                                                                                            </div>
                                                                                                        `;
                        });
                        ...
                        transactions.forEach(tr => {
                            ...
                            transHtml += `
                                                                                                            <div class="p-3 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-white/5 rounded-none mb-2 flex justify-between items-center transition-hover hover:bg-white dark:hover:bg-slate-700">
                                                                                                                ...
                                                                                                                <div class="text-right">
                                                                                                                    <span class="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">Rp ${new Intl.NumberFormat('id-ID').format(tr.total_harga || 0)}</span>
                                                                                                                </div>
                                                                                                            </div>
                                                                                                        `;
                        });
                    ...
                    const dashboardHtml = `
                                                                                                    <div class="flex flex-col h-full dark:bg-black overflow-hidden font-sans">
                                                                                                        ...
                                                                                                        <div class="flex flex-row gap-0 overflow-hidden" style="height: 500px;">
                                                                                                            <!-- Left: Items -->
                                                                                                            <div class="w-[30%] min-w-[200px] p-5 overflow-y-auto custom-scrollbar border-r border-gray-100 dark:border-white/5 bg-white dark:bg-black">
                                                                                                            ...
                                                                                                            <!-- Right: Transactions -->
                                                                                                            <div class="w-[70%] p-5 overflow-y-auto custom-scrollbar bg-gray-50/30 dark:bg-black" style="width:100%;">
                                                                                                                <h4 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                    ...
                    Swal.fire({
                        ...
                        customClass: { popup: 'rounded-none border-none overflow-hidden shadow-2xl' },
                        allowOutsideClick: true
                    });
                }
            } catch (e) {
                console.error(e);
                Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal memuat dashboard mitra.', customClass: { popup: 'rounded-none' } });
            }
        }
