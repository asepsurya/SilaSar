        ...
        <div class="flex min-w-0 items-center gap-3">
            <button class="flex items-start rounded-lg p-1 transition hover:bg-black/5 md:hidden dark:hover:bg-white/10"
                ...
                <div title="{{ $perusahaan_sidebar->nama_perusahaan }}"
                    class="h-8 w-8 cursor-default overflow-hidden rounded-full border border-gray-200 bg-white dark:border-white/20 dark:bg-transparent">
            ...
            <button type="button"
                class="hidden rounded-full p-2 text-black transition hover:bg-black/5 dark:text-white dark:hover:bg-white/10 md:flex"
                    ...
                    <ol class="flex min-w-0 items-center gap-1.5 overflow-hidden text-sm">
                        ...
                        @foreach ($segments as $index => $segment)
                            ...
                            <li class="flex min-w-0 items-center gap-1.5">
                                ...
                                @if ($isLinkable)
                                    <a href="{{ url($url) }}"
                                        class="max-w-[120px] truncate rounded-md px-2 py-1 text-black/45 transition hover:bg-black/5 hover:text-black dark:text-white/45 dark:hover:bg-white/10 dark:hover:text-white">
                                        {{ $label }}
                                    </a>
                                @else
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                <div class="flex items-center gap-1.5 rounded-full border border-black/5 bg-black/[0.02] p-1 dark:border-white/10 dark:bg-white/5">
                    <a href="javascript:;" class="flex h-9 w-9 items-center justify-center rounded-full text-black transition hover:bg-black/5 hover:text-black dark:text-white dark:hover:bg-white/10 dark:hover:text-white"
                        ...
                        <x-icon name="moon" class="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    </a>
                    <a href="javascript:;" class="flex h-9 w-9 items-center justify-center rounded-full text-black transition hover:bg-black/5 hover:text-black dark:text-white dark:hover:bg-white/10 dark:hover:text-white"
                        x-cloak x-show="$store.app.mode === 'dark'" @click="$store.app.toggleMode('light')"
                        aria-label="Ganti ke mode terang">
                        <x-icon name="sun" class="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    </a>
                </div>
                ...
                <button type="button" class="relative flex h-9 w-9 items-center justify-center rounded-full text-black transition hover:bg-black/5 hover:text-black dark:text-white dark:hover:bg-white/10 dark:hover:text-white"
                    ...
                    <x-icon name="notif" class="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    <span class="absolute right-[9px] top-[8px] flex h-2.5 w-2.5 items-center justify-center">
                        <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-black/50 opacity-75 dark:bg-white/70"></span>
                        <span class="relative inline-flex h-2 w-2 rounded-full bg-black dark:bg-white"></span>
                    </span>
                </button>

                @if (auth()->user()->role === 'gold')
                    <div class="inline-flex items-center gap-1.5 rounded-full bg-yellow-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-yellow-700 dark:bg-yellow-500/15 dark:text-yellow-300">
                        <i class="ph ph-crown-simple text-sm"></i>
                        Gold
                    </div>
                @elseif (auth()->user()->role === 'platinum')
                    <div class="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-blue-700 dark:bg-blue-500/15 dark:text-blue-300">
                        <i class="ph ph-crown-simple text-sm"></i>
                        Platinum
                    </div>
                @elseif (auth()->user()->role === 'admin')
                    <div class="inline-flex items-center gap-1.5 rounded-full bg-indigo-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300">
                        <i class="ph ph-crown-simple text-sm"></i>
                        Admin
                    </div>
                @elseif (auth()->user()->role === 'superadmin')
                    <div class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">
                        <i class="ph ph-crown-simple text-sm"></i>
                        Superadmin
                    </div>
                @endif
                ...
                <div class="profile" x-data="dropdown" @click.outside="open = false">
                    <button type="button"
                        class="flex items-center gap-2 rounded-full border border-black/5 bg-black/[0.02] px-2 py-1.5 transition hover:bg-black/5 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10"
                        @click="toggle()">
                        ...
                        <span class="hidden text-sm font-medium text-black dark:text-white xl:block"
                    ...
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms>
                            ...
                            <div class="flex items-center !p-1">
                                ...
                                <div class="pl-2">
                                    ...
                                    @php
                                        ...
                                        $maxLength = 18;
                                        $displayEmail = strlen($email) > $maxLength ? substr($email, 0, $maxLength) . '...' : $email;
                                    @endphp
                        ...
                        <li class="my-1 block h-px bg-black/5"></li>
                        ...
                        <li>
                            ...
                            <button type="button" onclick="confirmLogout()"
                                class="flex w-full items-center text-left text-black dark:text-white">
