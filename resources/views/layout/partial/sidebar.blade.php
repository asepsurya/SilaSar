    ...
    <div class="bg-white dark:bg-black h-full">
        ...
        <div class="flex items-center p-4 ">
            ...
            <div
                ...
                <img src="{{ $perusahaan_sidebar->logo ? asset('storage/' . $perusahaan_sidebar->logo) : asset('assets/default_logo.png') }}"
                    alt="logo" class="w-full h-full object-contain block dark:hidden" />

                <!-- Logo dark mode -->
                <img src="{{ $perusahaan_sidebar->logo ? asset('storage/' . $perusahaan_sidebar->logo) : asset('assets/default_logo.png') }}"
                    alt="logo" class="w-full h-full object-contain hidden dark:block" />
            </div>
        ...
        <ul class="relative h-[calc(100vh-58px)] flex flex-col gap-1 overflow-y-auto overflow-x-hidden p-4 py-0"
            ...
            <li class="menu nav-item"
                x-data="{ open: {{ in_array($active ?? '', ['dashboard', 'dahboardkeuangan', 'peta', 'catatan']) ? 'true' : 'false' }} }">

                <!-- Trigger utama -->
                <a href="javascript:;" class="nav-link group text-black dark:text-white" @click="open = !open"
                    :class="{ 'active': open }">
                ...
                <ul x-show="open" x-collapse class="sub-menu flex flex-col gap-1 text-black dark:text-white/80">
                    <li>
                        <a href="{{ route('dashboard') }}"
                            class="{{ $active === 'dashboard' ? 'active' : '' }}">Penjualan</a>
                    </li>
                    <li>
                        <a href="{{ route('dashboard.keuangan') }}"
                            class="{{ $active === 'dahboardkeuangan' ? 'active' : '' }}">Keuangan</a>
                    </li>
                    <li>
                        <a href="{{ route('dashboard.keuangan.harian') }}"
                            class="{{ $active === 'catatan' ? 'active' : '' }}">Keuangan Saya</a>
                    </li>
                    <li>
                        <a href="{{ route('dashboard.peta') }}"
                            class="{{ $active === 'peta' ? 'active' : '' }}">Peta</a>
                    </li>
                </ul>
            </li>
            <li class="menu nav-item">
            ...
            {{-- Data Keuangan --}}
            <li class="menu nav-item"
                x-data="{ open: {{ in_array($active ?? '', ['keuangan_harian', 'akun_harian', 'rekening_harian']) ? 'true' : 'false' }} }">
                ...
                <ul x-show="open" x-collapse class="sub-menu flex flex-col gap-1 text-black dark:text-white/80">
                    <li><a href="{{ route('index.keuangan.harian') }}"
                            class="{{ $active === 'keuangan_harian' ? 'active' : '' }}">Buku Kas</a></li>
                    <li><a href="{{ route('index.akun.harian') }}"
                            class="{{ $active === 'akun_harian' ? 'active' : '' }}">Akun</a>
                    </li>
                    <li><a href="{{ route('akun.rekening.harian') }}"
                            class="{{ $active === 'rekening_harian' ? 'active' : '' }}">Rekening</a></li>
                </ul>
            </li>
            <li class="menu nav-item"
                x-data="{ open: {{ in_array($active ?? '', ['keuangan', 'akun', 'rekening']) ? 'true' : 'false' }} }">
                <a href="javascript:;" class="nav-link group text-black dark:text-white" :class="{ 'active': open }"
                    @click="open = !open">
                    <div class="text-black/50 dark:text-white/20 w-4 h-4 flex items-center justify-center transition-transform duration-300"
                        :class="{ '!rotate-90': open }">
            ...
            <li class="menu nav-item"
                x-data="{ open: {{ in_array($active ?? '', ['laporan_transaksi', 'neraca', 'neracasaldo', 'labarugi']) ? 'true' : 'false' }} }">
                ...
                <!-- Submenu -->
                <ul x-show="activeMenu === 'laporan'" x-collapse x-transition
                    ...
                    <li>
                        <a href="{{ route('laporan.labarugi') }}" class="{{ $active === 'labarugi' ? 'active' : '' }}">
                            Laba Rugi
                        </a>
                    </li>
            ...
            <li class="menu nav-item"
                x-data="{ open: {{ in_array($active ?? '', ['add_produk', 'produk', 'category', 'satuan']) ? 'true' : 'false' }} }">
                <a href="javascript:;" class="nav-link group text-black dark:text-white" :class="{ 'active': open }"
                    @click="open = !open">
                    <div class="text-black/50 dark:text-white/20 w-4 h-4 flex items-center justify-center transition-transform duration-300"
                        :class="{ '!rotate-90': open }">
                ...
                <ul x-show="open" x-collapse class="sub-menu flex flex-col gap-1 text-black dark:text-white/80">
                    ...
                    <li><a href="{{ route('index.produk') }}" class="{{ $active === 'produk' ? 'active' : '' }}">Data
                            Produk</a></li>
            ...
            @if (auth()->check() && auth()->user()->role === 'admin' || auth()->user()->role === 'superadmin')
                ...
                <li class="menu nav-item"
                    x-data="{ open: {{ in_array($active ?? '', ['ikm', 'ikm_create', 'ikm_update']) ? 'true' : 'false' }} }">
                    <a href="javascript:;" class="nav-link group text-black dark:text-white" :class="{ 'active': open }"
                        @click="open = !open">
                        <div class="text-black/50 dark:text-white/20 w-4 h-4 flex items-center justify-center transition-transform duration-300"
                            :class="{ '!rotate-90': open }">
