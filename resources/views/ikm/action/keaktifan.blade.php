...
@section('container')
    ...
    <div x-data="{ tab: 'harian' }">
        ...
        <div class="flex space-x-2 bg-gray-100 dark:bg-white/5 p-3 w-max">
            <!-- Data Pengguna -->
            <a href="/people"
                class="flex items-center space-x-2 px-4 py-2 text-sm font-semibold rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400">
            ...
            <button @click="tab = 'harian'"
                :class="tab === 'harian' ? 'bg-blue-600 text-white' : 'bg-white/5 text-gray-800 dark:text-white/70'"
                class="flex items-center space-x-2 px-4 py-2 text-sm font-semibold rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
        ...
        <div x-show="tab === 'harian'" class="p-4 rounded-lg" x-data="dataKeaktifan" x-init="init()">
            <div
                class="flex flex-col md:flex-row items-start md:items-center justify-between border-gray-200 dark:border-white/10 px-5 mb-2">
                <!-- Judul kiri -->
                ...
                <div class="flex flex-col items-center w-full md:flex-row md:items-center md:w-auto gap-3">
                    ...
                    <div class="flex space-x-2 justify-center w-full md:w-auto md:justify-start">
                        ...
                        <button @click="loadData('harian')"
                            ...
                            class="px-4 py-2 text-sm font-semibold border-b-2 -mb-px transition
                                {{ $disableHarian ? 'cursor-not-allowed opacity-50' : '' }}" {{ $disableHarian ? 'disabled' : '' }}>
                            Harian
                        </button>

                        <button @click="loadData('mingguan')"
                            :class="periodeAktif === 'mingguan' ? 'active-button border-b-2 border-blue-500 text-blue-600' : 'border-transparent text-gray-500'"
                            class="px-4 py-2 text-sm font-semibold border-b-2 -mb-px transition">
                            Mingguan
                        </button>
                        <button @click="loadData('bulanan')"
                    ...
                    <form id="filterForm" method="GET"
                        class="flex gap-0 items-center justify-center w-full md:w-auto md:justify-start">
            ...
            <div class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 p-2 rounded-md">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    <div class="col-span-12 md:col-span-8">
                        <div
                            class="border bg-white dark:bg-black border-black/10 dark:border-white/10 rounded-md p-3 overflow-x-auto">
