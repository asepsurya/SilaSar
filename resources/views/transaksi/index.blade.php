...
@section('container')
    ...
    <div x-data="modals">
        <div class="px-2 py-1 mb-4 flex items-center justify-between">
            ...
            <button class="px-2 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition"
                @click="toggle">
        ...
        <div class="fixed inset-0 bg-black/60 dark:bg-white/10 z-[999] hidden overflow-y-auto" :class="open && '!block'">
            <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                <div x-show="open" x-transition="" x-transition.duration.300=""
                    ...
                    style="display: none; height: 500px; max-height: 500px;">
                    <div
                        ...
                        <button type="button"
                            class="text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white" @click="toggle">
                            ...
                            <form action="{{ route('transaksi.create') }}" method="POST">
                                ...
                                <div class="mb-4" hidden>
                                    <input type="text" placeholder="User Name"
                                        ...
                                        readonly name="kode_transaksi" id="kode_mitra">
                                </div>
                                <div class="mb-4">
                                    <input type="text" id="searchMitra" placeholder="Cari Nama Mitra / Toko... "
                                        class="form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;">
                                </div>
                                <script>
                                ...
                                <div
                                    class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 p-2 rounded-md mb-4">
                                    <label class="block text-xs text-black/40 dark:text-white/40 mb-2">Pilih Mitra</label>
                                    <div style="height:220px;overflow-y:auto;">
                                        <table
                                            class="min-w-full bg-white dark:bg-transparent border border-black/10 rounded-lg"
                                            id="tableMitra">
                                            <thead>
                                                <tr>
                                                    <th class="py-2 px-4 border-b text-left text-xs font-semibold">Pilih
                                                    </th>
                                                    <th class="py-2 px-4 border-b text-left text-xs font-semibold">Nama
                                                        Mitra</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($mitra as $item)
                                                    <tr>
                                                        <td class="py-2 px-4 border-b">
                                                            <input type="radio" name="kode_mitra"
                                                                value="{{ $item->kode_mitra }}" required>
                                                        </td>
                                                ...
                                                @if($mitra->isEmpty())
                                                    <tr>
                                                        <td colspan="2" class="py-4 px-4 text-center text-gray-500">Tidak ada
                                                            data mitra tersedia.</td>
                                                    </tr>
                                                @endif
                                            </tbody>
                                        </table>
    ...
    <div class="relative w-full mb-6 block md:hidden">
        ...
        <!-- Tombol panah -->
        <button class="absolute left-0 top-1/2 -translate-y-1/2 text-black text-3xl px-3 py-2 rounded-r-lg"
            onclick="plusDivs(-1)">&#10094;</button>
        <button class="absolute right-0 top-1/2 -translate-y-1/2 text-black text-3xl px-3 py-2 rounded-l-lg"
            onclick="plusDivs(1)">&#10095;</button>
    </div>
    <script>
        var slideIndex = 1;
    ...
    <div class="grid grid-cols-1 gap-7">
        {{-- Simple DataTable --}}
        <div x-data="main" x-init="init()"
            class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 p-2 rounded-md">
            <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold">Daftar Transaksi</p>
                    <p class="text-xs text-black/60 dark:text-white/60">Berikut adalah daftar transaksi mitra dan toko yang
                        telah tercatat di sistem.</p>
                </div>
                                ...
                                <div x-show="filterOpen" x-transition x-transition.duration.300
                                    ...
                                    <div class="p-5 max-h-[420px] overflow-y-auto">
                                        <form method="GET" action="{{ route('transaksi.index') }}"
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-black/60 dark:text-white/60 mb-1">Status
                                                    Pembayaran</label>
                                                <select name="status_pembayaran"
                                                    ...
                                                    <option value="belum_bayar" {{ request('status_pembayaran') == 'belum_bayar' ? 'selected' : '' }}>
                                                        Belum Bayar</option>
                                                    <option value="sudah_bayar" {{ request('status_pembayaran') == 'sudah_bayar' ? 'selected' : '' }}>
                                                        Sudah Bayar</option>
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-black/60 dark:text-white/60 mb-1">Periode
                                                    Waktu</label>
            ...
            <div class="clean-table-container hidden sm:block">
            ...
            <div class="block md:hidden">
                    ...
                    @foreach($transaksimobile as $item)
                        <div class="bg-white dark:bg-black border border-black/10 dark:border-white/10 rounded-lg mb-4 p-4 shadow cursor-pointer"
                            ...
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-sm font-bold transaction-code">
                                    {{ $item->kode_transaksi }}
                                </span>
                            </div>
                            <div class="grid grid-cols-2 gap-2 mb-2">
                                <div>
                                    ...
                                    <span
                                        class="block text-sm font-medium text-black dark:text-white">{{ \Carbon\Carbon::parse($item->tanggal_transaksi)->translatedFormat('d M Y') }}
                                    </span>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 text-xs font-semibold
                                                                                                    {{ $item->status_bayar == 'Sudah Bayar' ? 'text-green-600' : 'text-red-600' }}">
                ...
                @endphp
                @if($totalPages > 1)
                    <div class="datatable-pagination flex justify-center mb-4">
                        <ul>
                            @if($currentPage > 1)
                                <li>
        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data('main', () => ({
                ...
                , loadData() {
                    ...
                    // Tambahkan class agar styling kita masuk
