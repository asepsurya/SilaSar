...
<head>
    ...
    <style>
        ...
        .col-no { width: 30px; }
        .col-qty, .col-unit, .col-empty, .col-empty2 { width: 40px; }
        ...
        .stamp, .ttd { position: absolute; width: 120px; left: 50%; transform: translateX(-50%); }
        .stamp { opacity: 0.7; }
        .ttd { top: 20px; }
        ...
        textarea:focus { border: none; outline: none; box-shadow: none; }
        @page { margin: 100px 40px 100px 40px; size: A4; }
        footer {
            position: fixed;
            bottom: -60px;
            left: 0; right: 0;
    ...
    <div class="max-w-[900px] mx-auto p-6 text-black">
        ...
        <div class="text-[12px] leading-[14px] mb-3">
            <div class="flex space-x-2 text-[11px] leading-[13px]">
                <div class="w-25 font-bold" style="{{ request('type')=='invoice' ? 'width:100px' : '' }}">
                    {{ request('type')=='kwitansi' ? 'Telah Diterima dari' : 'Kepada' }}
                </div>
                <div class="w-1">:</div>
