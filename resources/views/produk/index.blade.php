...
@section('container')
    ...
    <div class="px-2 py-1 mb-2 flex items-center justify-between">
        <h2 class="text-lg font-semibold">Produk Saya <span
                class="px-1 bg-lightgreen-100 text-xs text-black rounded ml-1">{{ $produk->count() }}</span></h2>
        <a href="{{ route('index.create.produk') }}"
    ...
    <div class="a flex flex-col md:flex-row items-center gap-3 py-3">
        ...
        <div class="w-full cari">
            <input type="text" id="searchInput" placeholder="Cari Produk..." class="form-input py-2.5 px-4 w-full text-sm text-gray-700 
        ...
        <form action="" method="GET" class="w-full filter ">
            <button type="button" @click="window.dispatchEvent(new CustomEvent('filter'))" class=" w-full flex items-center justify-center gap-x-2 
                ...
                <div x-show="open" x-transition x-transition.duration.300
                    ...
                    <div class="p-5">
                        <form method="GET" class="flex flex-col gap-5">
                            ...
                            <!-- Filter Kategori -->
                            <div>
                                <label class="block text-xs font-semibold text-black/60 dark:text-white/60 mb-1">Kategori
                                    Produk</label>
    ...
    <div class="clean-table-container">
        <div class="table-responsive">
            <table class="clean-table" id="produkTable">
                <thead>
                    <tr>
                        <th class="pl-6 py-3 font-normal">Product</th>
                        <th class="font-normal mobile">Created at</th>
                        <th class="font-normal pr-6 mobile">Amount</th>
                        <th class="w-6 "></th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr class="hover:bg-gray-50 border-b border-black/10 dark:border-white/10 cursor-pointer transition">
                            ...
                            <td class="py-4 pl-6 flex items-start gap-3">
                                <a data-fancybox="gallery" data-caption="{{ $item->nama_produk }}"
                                    href="{{ $item->first_image ? asset('storage/' . $item->first_image) : asset('assets/images/404.jpg') }}">
                                    <img src="{{ $item->first_image ? asset('storage/' . $item->first_image) : asset('assets/images/404.jpg') }}"
                                        alt="Produk" class="h-12 w-12 rounded-lg object-cover flex-shrink-0"
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="lg:hidden mt-2 text-xs text-gray-500 space-y-1">
                                        ...
                                        <div><strong>Stok:</strong> ({{$item->stok ?? '0'}}) {{ $item->satuanObj->nama ?? '-' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="text-right pr-4 relative" x-data="{ open: false }">
                                <button @click="open = !open"
                                    class="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <a href="{{ route('index.update.produk', $item->id) }}" class="block w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 
                                                            hover:bg-gray-100 dark:hover:bg-white/10 transition">
                                        Edit Produk
                                    </a>
                    ...
                    @if ($produk->isEmpty())
                        <tr>
                            <td colspan="5" class="text-center py-8 text-gray-500 items-center flex flex-col justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 mb-3 text-gray-400" fill="none"
        ...
        <div class="mt-0">
            {{ $produk->links('vendor.pagination.clean') }}
        </div>
    </div>
    <script>
                    ...
                    <div x-show="open" x-transition x-transition.duration.300
                        ...
                        <div class="p-5 space-y-5">
                            <p class="text-sm  dark:text-gray-300">
                                Produk <strong>{{ $err['nama'] }}</strong> tidak dapat dihapus karena masih digunakan pada data
                                berikut:
                            </p>
                        ...
                        <!-- Footer -->
                        <div
                            class="flex justify-end bg-gray-50 dark:bg-black/30 px-5 py-3 border-t border-black/10 dark:border-white/10">
                            <button type="button"
