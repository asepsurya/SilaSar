...
<form action="{{ route('manajemenStok.create') }}" method="POST">
    @csrf
@if ($errors->any())
<div class="flex items-start rounded bg-lightyellow/50 dark:bg-lightyellow p-3 text-black/80 dark:text-black">
    <!-- Icon -->
        ...
        <div class="flex flex-col lg:flex-row items-start justify-between gap-6 mb-4 border border-black/10 dark:border-white/10 p-5 rounded-md">
            ...
            <div class="w-full lg:w-1/2 space-y-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" value="2025-09-19" name="tanggal" class="form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg">
                </div>
            </div>
            ...
            <div x-show="open" x-transition @click.self="open = false" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 dark:bg-white/10 ">
                <div class="bg-white dark:bg-black rounded-xl shadow-2xl p-5 w-full max-w-lg dark:border-white/10">
                    ...
                    <template x-if="filtered.length > 0">
                        <div class="overflow-x-auto max-h-[500px] overflow-y-auto border dark:border-white/10  rounded-lg">
                            <table class="w-full  dark:border-white/10 text-sm min-w-[500px]">
                    ...
                    <template x-if="filtered.length === 0">
                        <p class="text-center text-gray-400 p-5">Tidak ada produk ditemukan</p>
                    </template>
                </div>
            </div>
        </div>
                            ...
                            <thead class="sticky top-0 bg-gray-100 dark:bg-black z-10">
                                <tr class="text-center">
                                   <th class="border dark:border-white/10 px-2 py-1 w-10">No</th>
                                    ...
                                    <th class="border dark:border-white/10 px-2 py-1 w-40">Nama Produk</th>
                                    <th class="border dark:border-white/10 px-2 py-1 w-28">Stok</th>
                                    <th class="border dark:border-white/10 px-2 py-1 w-24">Satuan</th>
                                    <th class="border dark:border-white/10 px-2 py-1 w-32">Harga</th>
                                    <th class="border dark:border-white/10 px-2 py-1 w-32">Total</th>
                                    <th class="border dark:border-white/10 px-2 py-1 w-14">Aksi</th>
            ...
            <div class="flex flex-col lg:flex-row gap-6 mt-4">
                ...
                <div class="w-full lg:w-1/2 space-y-5">
                    ...
                    <div class="bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg p-4">
                        ...
                        <textarea name="deskripsi" placeholder="Masukan Catatan disini.." class="form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;" rows="3" oninput="autoResize(this)"></textarea>
                    </div>
                ...
                <div class="w-full lg:w-1/2 bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg p-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                        ...
                        <input id="potongan" type="text" name="potongan" class="form-input py-2 px-4 w-full text-right rounded-lg border border-black/10 dark:border-white/10 dark:bg-transparent text-black dark:text-white" value="0">
        ...
        tr.innerHTML = `
            ...
            <td class="border dark:border-white/10 px-2 py-1">
                <input type="text" name="items[${no - 1}][nama_produk]"
                    ...
                    class="nama_produk border-0 w-full form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg" readonly>
            </td>
            <td class="border dark:border-white/10 px-2 py-1">
                <input type="number" name="items[${no - 1}][jumlah]"
                    value="${data.jumlah ?? 1}"
                    class="jumlah text-center border-0 w-full form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg">
            </td>
            <td class="border dark:border-white/10 px-2 py-1">
                <select name="items[${no - 1}][satuan_id]"
                        class="satuan border-0 w-full form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg">
                    ${satuanOptions}
                </select>
            </td>
            <td class="border dark:border-white/10 px-2 py-1">
                <input type="number" name="items[${no - 1}][harga]"
                    value="${data.harga ?? 0}"
                    class="harga border-0 w-full form-input py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg">
            </td>
            <td class="border dark:border-white/10 px-2 py-1 total text-center">0</td>
